...
这个离线包用于在另一台 Windows 10 x64 + RTX 3090 电脑上安装第一版深度学习环境：
...
第一版只支持 `Download / Check / Install / Verify`。`Git / CUDA Toolkit / Visualization / RegisterLocalFiles / Doctor / ReuseVenv / 多 Profile` 都放到第二阶段。

## 联网电脑推荐流程

1. 双击 `OfflineDL-Win10-3090.bat`。
2. 选择 `Download`，按提示输入 `y` 确认下载到当前脚本所在文件夹。
3. 如果提示缺少 NVIDIA 驱动，请从 NVIDIA 官方驱动页面下载 RTX 3090 / Windows 10 x64 驱动 exe，放到 `downloads\drivers`，再重新运行 `Download`。
4. 下载完成后选择 `Check`。
5. Check 通过后，把整个 `Win10_3090_DeepLearning_OfflinePack` 文件夹拷贝到离线电脑。
...
本脚本第一版不会自动静默安装 NVIDIA 驱动、Python 安装包、VC++ Runtime。第一次安装时请按提示手动运行对应安装包。
...
第一版不支持 `-Profile`、`-ReuseVenv`、`-IncludeGit`、`-IncludeCudaToolkit`、`-IncludeVisualization`。传入这些参数会直接提示第二阶段暂不可用。
